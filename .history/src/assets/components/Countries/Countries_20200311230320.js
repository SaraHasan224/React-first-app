...
class Countries extends Component {
    constructor(){
        this._handleDe_initCountrieslete = this._initCountries.bind(this);
        super();
        this.setState = {
            "allCountries": [],
    ...
    _initCountries(){
        ...
        fetch(CONFIG.get_countries).then((resp) => resp.json()).then(function (data){
            this.setState( (state) => ( { allCountries: data}) );
        })
        .catch(function(error){
            console.log(error);
            this.setState( (state) => ( { countryListError: true}) );
        });
    }
